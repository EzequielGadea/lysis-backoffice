...
<x-layout>
    ...
    <div class="flex flex-col items-center pt-6 px-8 flex-grow">
        ...
        <div class="flex flex-col gap-12">
                ...
                <table class="table-auto border-collapse w-full whitespace-nowrap">
                    <thead>
                        <tr class="bg-slate-300">
                            ...
                            <td class="p-3 font-light text-zinc-800">CARD</td>
                    ...
                    <tbody>
                        @foreach ($event->playerLocal->cardSanctions->sortBy(['minute', 'desc']) as $sanction)
                            <tr class="border-b border-slate-300">
                                <td class="p-3 text-zinc-800">{{ $sanction->id }}</td>
                                <td class="p-3 text-zinc-800">{{ $sanction->minute }}'</td> 
                                @if ($event->result()->result_type_id == '3')
                                    <td class="p-3 text-zinc-800">Set {{ $sanction->inSet->set->number ?? 'no tiene' }}</td>
                                @endif
                                <td class="p-3 text-zinc-800">
                                    <div class="flex flex-row gap-4 items-center">
                                        <div style="background-image: url({{ asset('images/' . $sanction->player->player->picture) }})"
                                        ...
                                        <p>
                                            {{ $sanction->player->player->name  ?? 'no tiene'}}
                                            {{ $sanction->player->player->surname ?? 'no tiene'}}
                                        </p>
                                    </div>
                                </td>
                                <td class="p-3 text-zinc-800">
                                    {{ $sanction->sanction->color}}
                                </td>
                                <td class="p-3 text-zinc-800">
                                    {{ $sanction->created_at }}
                                </td>
                                ...
                                <td class="p-3 text-zinc-800">
                                    <div class="flex flex-col items-center">
                                        <form action="/sanctions/cards/individual/local/edit/{{ $sanction->id }}" method="GET">
                                            <button type="submit" class="font-semibold text-blue-600">Edit</button>
                                        </form>
                                        <form action="/sanctions/cards/individual/local/delete/{{ $sanction->id  }}" method="post">
                                            @csrf
                                            @method('delete')
            ...
            <div class="rounded-md overflow-x-auto shadow-xl w-[75rem]">
                <table class="table-auto border-collapse w-full whitespace-nowrap">
                    ...
                    <tbody>
                        @foreach ($event->playerVisitor->cardSanctions->sortBy(['minute', 'desc']) as $sanction)
                            <tr class="border-b border-slate-300">
                                <td class="p-3 text-zinc-800">{{ $sanction->id }}</td>
                                <td class="p-3 text-zinc-800">{{ $sanction->minute }}'</td> 
                                @if ($event->result()->result_type_id == '3')
                                ...
                                <td class="p-3 text-zinc-800">
                                    <div class="flex flex-col items-center">
                                        <form action="/sanctions/cards/individual/visitor/edit/{{ $sanction->id }}" method="GET">
                                            <button type="submit" class="font-semibold text-blue-600">Edit</button>
                                        </form>
                                        <form action="/sanctions/cards/individual/visitor/delete/{{ $sanction->id  }}" method="post">
                                            @csrf
                                            @method('delete')
                                            <button type="submit" class="font-semibold text-blue-600">Delete</button>
                                        </form>
        ...
        @if (session('statusDelete') && session('deletedId'))
            <x-status-delete-get>
                <x-slot name="action">/sanctions/cards/individual/{{ session('side') }}/restore/{{ session('deletedId') }}</x-slot>
            </x-status-delete-get>
        @endif
    ...
    <x-create-section>
        <x-slot name="action">sanctions/cards/individual/create/{{ $event->id }}</x-slot>
            ...
            <select name="sanction" class="w-64 bg-slate-200 px-3 py-1 rounded-md shadow-inner" autocomplete="off">
                ...
                @foreach($cards as $card)
                    <option value="{{ $card->id }}" 
                        @if (old('sanction') == $card->id) selected @endif
                    >{{ $card->color }}</option>
                @endforeach
            </select>
