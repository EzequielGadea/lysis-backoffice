<x-layout>
    ...
    <div class="flex flex-col items-center pt-6 px-8 flex-grow">
                    ...
                    <tbody>
                        @foreach ($event->opponents() as $team)
                            @foreach ($team->events as $player)
                                @foreach ($player->cardSanctions->sortBy(['minute', 'desc']) as $sanction)
                            <tr class="border-b border-slate-300">
                                ...
                                <td class="p-3 text-zinc-800">
                                    <div class="flex flex-col items-center">
                                        <form action="/sanctions/cards/team/edit/{{ $sanction->id }}" method="GET">
                                            <button type="submit" class="font-semibold text-blue-600">Edit</button>
                                        </form>
                                        <form action="/sanctions/cards/team/delete/{{ $sanction->id  }}" method="post">
                                            @csrf
                                            @method('delete')
        ...
        @if (session('statusDelete') && session('deletedId'))
            <x-status-delete-get>
                <x-slot name="action">/sanctions/cards/team/restore/{{ session('deletedId') }}</x-slot>
            </x-status-delete-get>
        @endif
    ...
    <x-create-section>
        <x-slot name="action">sanctions/cards/team/create/{{ $event->id }}</x-slot>
