<div class="flex flex-col gap-3">
    <div class="flex flex-col gap-1">
        ...
        <select wire:model="resultTypeId" name="resultTypeId" id="resultTypeId"
            class="w-64 bg-slate-200 px-3 py-1 rounded-md placeholder:text-zinc-600 shadow-inner" autocomplete="off">
            <option value="" selected disabled>-- Choose result type --</option>
            @foreach ($resultTypes as $type)
                <option value="{{ $type->id }}" @if (old('resultTypeId') == $type->id) selected @endif>
                    {{ $type->name }}
                </option>
            @endforeach
        </select>
    ...
    @if ($resultTypeId == 1)
        <div class="flex flex-col gap-1">
            ...
            <select name="markNameId" id="markNameID" class="w-64 bg-slate-200 px-3 py-1 rounded-md shadow-inner"
                autocomplete="off">
                <option selected disabled>-- Choose mark name --</option>
                @foreach ($markNames as $markName)
                    <option value="{{ $markName->id }}" @if (old('markNameId') == $markName->id) selected @endif>
                        {{ $markName->name }}
                    </option>
                @endforeach
            </select>
    ...
    @if ($resultTypeId == 3)
        <div class="flex flex-col gap-1">
            ...
            <input type="number" min="1" name="setAmount" id="setAmount" placeholder="Enter amount of sets"
                value="{{ old('setAmount') }}"
