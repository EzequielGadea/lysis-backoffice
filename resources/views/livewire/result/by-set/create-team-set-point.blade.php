<form action="/set/team/create/{{ $result->id }}" method="post" class="bg-slate-50 flex flex-col gap-6 w-80 px-8 py-6">
    @csrf
    ...
    <div class="flex flex-col gap-1">
        ...
        <select name="set" autocomplete="off" class="w-64 bg-slate-200 px-3 py-1 rounded-md shadow-inner">
            ...
            @foreach ($result->sets as $set)
                <option value="{{ $set->id }}">{{ $set->number }}</option>
            @endforeach
        </select>
    </div>
    ...
    <div class="flex flex-col gap-1">
        ...
        <input type="number" name="minute" placeholder="Enter minute"
    ...
    <div class="flex flex-col gap-1">
        ...
        <input type="number" name="points" placeholder="Enter amount of points"
            class="w-64 bg-slate-200 px-3 py-1 rounded-md 
                 placeholder:text-zinc-600 shadow-inner" />
        <p class="text-sm text-red-600">
            {{ $errors->first('points') }}
    ...
    <div>
        <input type="hidden" name="isInFavor" value="0">
        <input type="checkbox" name="isInFavor" value="1" checked
            class="w- bg-slate-200 px-3 py-1 rounded-md 
                   placeholder:text-zinc-600 shadow-inner" />
        <label for="isInFavor" class="font-medium text-zinc-700">Is in favor</label>
        <p class="text-sm text-red-600">
            {{ $errors->first('point') }}
        </p>
    </div>
