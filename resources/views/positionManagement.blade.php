<x-layout>
    <x-slot name="title">Position management</x-slot>
    <x-nav/>
    <div class="flex flex-col items-center pt-6 px-8 w-full flex-grow">
        <p class="text-2xl text-zinc-800 font-semibold mb-6 w-full">Positions</p>
        <div class="rounded-md overflow-x-auto shadow-xl w-full">
            <table class="table-auto border-collapse w-full">
                <thead>
                    <tr class="bg-slate-300">
                        <td class="pl-3 py-3 font-light text-zinc-800">POSITION ID</td>
                    ...
                    @if (isset($positions))
                        @foreach ($positions as $position)
                            <tr class="border-b border-slate-300">
                                <td class="pl-3 text-zinc-800">{{ $position->id }}</td>
                                <td class="py-3 text-zinc-800">{{ $position->name }}</td>
                                <td class="py-3 text-zinc-800">{{ $position->created_at }}</td>
                                <td class="py-3 text-zinc-800">{{ $position->updated_at }}</td>
                                <td class="pr-3 text-zinc-800">
                                    <div class="flex flex-col items-center">
                                        <form action="positionUpdate/{{ $position->id }}" method="GET">
        ...
        @if(session('statusDelete') && session('deletedId'))
            <x-status-delete>
                <x-slot name="action">positionRestore</x-slot>
    ...
    <x-create-section>
        <x-slot name="action">positionRegister</x-slot>
