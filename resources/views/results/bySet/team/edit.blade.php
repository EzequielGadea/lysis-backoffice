<x-layout>
    ...
    <x-update-card>
        <x-slot name="action">set/team/update/{{ $point->id }}</x-slot>
        <x-slot name="backTo">set/team/index/{{ $point->set->result->event->id }}</x-slot>
        ...
        <div class="flex flex-col gap-1">
            ...
            <div>
                <input type="number" name="minute" placeholder="Enter minute" value="{{ $point->minute }}"
        ...
        <div class="flex flex-col gap-1">
            ...
            <div>
                @if ($point->points_in_favor == 0)
                    <input type="number" name="points" placeholder="Enter points" value="{{ $point->points_against }}"
                        class="w-64 bg-slate-200 px-3 py-1 rounded-md placeholder:text-zinc-600 shadow-inner">
                @else
                    <input type="number" name="points" placeholder="Enter points"
                        value="{{ $point->points_in_favor }}"
                        class="w-64 bg-slate-200 px-3 py-1 rounded-md placeholder:text-zinc-600 shadow-inner">
                @endif
            </div>
