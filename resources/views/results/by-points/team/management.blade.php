<x-layout>
    ...
    <div class="flex flex-col items-center pt-6 px-8 flex-grow">
        ...
        <div class="flex flex-col gap-12">
            <div class="rounded-md overflow-x-auto shadow-xl w-[75rem]">
                <table class="table-auto border-collapse w-full whitespace-nowrap">
                    ...
                    <tbody>
                        @foreach ($event->result()->teamPoints()->get()->sortBy([['minute', 'desc']]) as $point)
                            <tr class="border-b border-slate-300">
                                ...
                                <td class="p-3 text-zinc-800">
                                    <div class="flex flex-col items-center">
                                        <form action="/points/team/edit/{{ $point->id }}" method="GET">
                                            <button class="font-semibold text-blue-600">Edit</button>
                                        </form>
                                        <form action="/points/team/delete/{{ $point->id }}" method="post">
                                            @csrf
                                            @method('delete')
        ...
        @if (session('statusDelete') && session('deletedId'))
            <x-status-delete-get>
                <x-slot name="action">/points/team/restore/{{ session('deletedId') }}</x-slot>
            </x-status-delete-get>
        @endif
    ...
    @livewire('result.by-points.individual.create-point-form', ['result' => $event->result()])
</x-layout>
